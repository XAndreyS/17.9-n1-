...
if True:
    try:#Исключения для ошибки RecursionError
        if count_number == 1:   #если нет повторяющихся чисел запрашиваемых пользователем в списке numbers,
            ...
            print(f'ИндекС предыдущего Числа = {binary_seearch(for_index_number, number, min_index, max_index) }') #вызываем функцию бинарного поиска введённого пользователем числа и увеличиваем на 1(пока исправлял возможные ошибки, так получилось)
